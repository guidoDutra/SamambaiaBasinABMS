...
species Crop {
	...
	float sell_price update: monthly_prices[month_count + 2, crop_number] as float;
	...
	action calculate_expected_water_demand type: float {
		...
		int initial_day <- year_count > 0 ? cycle -360 : cycle; // use previous year data if available, current year data otherwise
		int i <- 0;
	...
	action  calculate_expected_revenue type: float {
		int initial_day<- year_count > 0 ? cycle - 360 : cycle;
		int harvest_day <- initial_day + stage_change_days[3];
		int harvest_month <- floor(harvest_day / 30) as int;
		float expected_sell_price <- float(monthly_prices[harvest_month + 2, crop_number]);
	...
	action calculate_evapotranspiration(int month) type: float {
		...
		return 0.00017136 * (month_max_temp - month_min_temp) ^ (0.96) * (((month_max_temp + month_min_temp) / 2) + 17.38);
	}
	...
	action calculate_expected_rain (
		...
		// discounts month average rain * stage duration * (1 - rain variation coeficient)
	 	float expected_rain <- float(monthly_weather[3, reference_month]) * stage_duration;
